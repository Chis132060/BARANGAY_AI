...
export function TopBar() {
  ...
  async function markRead(id: string) {
    setNotifications((items) =>
      items.map((n) => (n.id === id ? { ...n, read_status: true } : n))
    );
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-gray-100
      ...
      <Link href="/home" className="flex items-center gap-2">
        <img src="/logo.png" alt="Barangay Logo" className="h-7 w-auto object-contain" />
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute right-0 top-11 z-50 w-72 rounded-2xl border border-gray-200 bg-white shadow-xl overflow-hidden">
            ...
            <div className="max-h-72 overflow-y-auto">
              {notifications.length === 0 ? (
                <p className="p-5 text-center text-xs text-gray-500">No new notifications</p>
              ) : (
                notifications.map((n) => (
                  <button
                    key={n.id}
                    onClick={() => markRead(n.id)}
                    className={`w-full text-left border-b px-4 py-3 hover:bg-blue-50 ${!n.read_status ? "bg-blue-50/60" : ""}`}
                  >
            ...
            <Link
              ...
              className="block border-t p-3 text-center text-xs font-bold text-blue-600"
            >
              View all updates
            </Link>
          </div>
